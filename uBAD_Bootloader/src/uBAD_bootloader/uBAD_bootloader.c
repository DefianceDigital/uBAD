...
#define INCLUDE_FROM_UBAD_C
#include "uBAD_bootloader.h"
...
int main(void)
{
    ...
    *bootKeyPtr = 0;
    ...
    GPIOR0 = mcusr_state; // use GPIO0 register to preserve MCUSR for application use
    ...
    if((failedCount > 0x01) && (failedCount < 0xFF)){ // triggered by failed reValidate() in application
      ...
      for(uint16_t i = 2; i < 1024; i++){
        eeprom_update_byte((uint8_t *)(intptr_t)i, 0xFF); // set all bytes to 0xFF that aren't already
      }
